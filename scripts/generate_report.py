...
| **Half-Life** | {a['half_life'].get('half_life', float('inf')):.1f} períodos | — | {a['half_life'].get('interpretation', '')} |
...
"""
    ...
    for sym in symbols:
        ...
        bullish_signals = sum([
            a['rsi'] < 30,  # oversold = bullish
            a['macd'] > a['macd_signal'],  # MACD bullish
            bb_pos < 0.2,  # near lower band = bullish
...
- **Half-Life (OU Process):** Tempo para reverter 50% do choque ao média.
...
4. Mandelbrot & Wallis (1969). Robustness of the rescaled range.
...
def main():
    ...
    parser.add_argument("--json-output", type=str, help="Also save analysis as JSON")
